...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        cap_after_pos = self.positional_encoding(self.embedding(captions.long()))
        fv = self.visual_projection(features)
        mask = torch.tril(torch.ones(T, T, dtype=bool)).T
        fv = fv.unsqueeze(1)
        out = self.transformer(cap_after_pos, fv, mask)
