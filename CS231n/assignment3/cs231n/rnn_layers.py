...
def rnn_step_backward(dnext_h, cache):
    ...
    N, D = x.shape
    dmid = (dnext_h * (1 - np.square(next_h)))
    dx = dmid @ Wx.T
    dprev_h = dmid @ Wh.T
    dWx = x.T @ dmid
    dWh = prev_h.T @ dmid
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    temp_h = h0.copy()
    for t in range(T):
        temp_h, _ = rnn_step_forward(x[:, t], temp_h, Wx, Wh, b)
        h[:, t] = temp_h
...
def rnn_backward(dh, cache):
    ...
    D = x.shape[2]
    dh_ = dh.swapaxes(0, 1)
    h_ = np.vstack((h0[np.newaxis, :], h.swapaxes(0, 1)))
    dx = np.zeros((T, N, D))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    db = np.zeros(H)
    ...
    for t in range(T - 1, -1, -1):
        cache = (x[:, t], h_[t], h_[t + 1], Wx, Wh, b)
        dx[t], dh_begin, tdWx, tdWh, tdb = rnn_step_backward(dh_begin + dh_[t], cache)
        dWx += tdWx
        dWh += tdWh
        db += tdb
    dh0 = dh_begin
    ...
    for t in range(T - 1, -1, -1):
        ...
        for i in range(t, -1, -1):
            cache = (x[:, i], h_[i], h_[i + 1], Wx, Wh, b)
            tdx[i], dh_begin, tdWx, tdWh, tdb = rnn_step_backward(dh_begin, cache)
            dWx += tdWx
            dWh += tdWh
            db += tdb
        dx += tdx
