...
def get_posterior_params(X, y_train, prior, likelihood_var=0.2**2):
    ...
    Args:
        ...
        likelihood_var: likelihood variance- default (0.2**2)
    ...
    '''
    post_mean = (X.T @ X + likelihood_var *
                 np.matrix.getI(prior['var'])) @ X.T @ y_train
    post_var = np.matrix.getI(X.T @ X / likelihood_var +
                              np.matrix.getI(prior['var']))
...
if __name__ == '__main__':
    ...
    If your implementations are correct, running
        python problem.py
    inside the Bayesian Regression directory will, for each sigma in
    sigmas_to-test generates plots
    '''
