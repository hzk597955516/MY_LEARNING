...
def likelihood_func(w, X, y_train, likelihood_var):
    ...
    #TO DO
    n = X.shape[0]
    right = np.exp(1 / (-2 * likelihood_var) * np.sum(np.square(y_train - X @ w)))
    left = 1 / np.power(2 * likelihood_var * np.pi, n / 2)
    likelihood = right * left
    return likelihood
...
def get_posterior_params(X, y_train, prior, likelihood_var = 0.2**2):
    ...
    # TO DO
    pvarI = prior["var"].I
    post_var = ((X.T @ X) / likelihood_var + pvarI).I
    post_mean = post_var @ ((1 / likelihood_var) * X.T @ y_train + pvarI.T @ prior["mean"])
...
if __name__ == '__main__':
    ...
    actual_weights = np.matrix([[1.3], [1.5]])
    data_size = 40
