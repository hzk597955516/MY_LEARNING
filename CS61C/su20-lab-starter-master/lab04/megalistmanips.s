...
mapLoop:
    ...
    li t3, 4
    mul t3, t3, t0
    add t3, t1, t3      # offset the array address by the count(change part 2)
    lw a0, 0(t3)        # load the value at that address into a0

    addi sp, sp, -4
    sw t1, 0(sp) # change part 1

    jalr s1             # call the function on that value.

    lw t1, 0(sp)
    addi sp, sp, 4 # change part 1

    sw a0, 0(t3)        # store the returned value back into the array
    addi t0, t0, 1      # increment the count
    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet

    lw a0, 8(s0)        # load the address of the next node into a0(change part 3)
