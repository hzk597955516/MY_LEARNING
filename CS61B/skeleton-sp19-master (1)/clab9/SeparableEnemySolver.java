...
public class  SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        ...
        for (String label: g.labels()){
            if (!colors.containsKey(label)){
                if (!isright(label, colors, 1, label)){
                    return false;
                }
            }
        }
    ...
    private boolean isright(String label, HashMap<String, Integer> colors, Integer color, String parent){
        if (colors.containsKey(label)){
        ...
        for (String neighbor: g.neighbors(label)){
            if (!neighbor.equals(parent)){
                if (!isright(neighbor, colors, -1 * color, label)){
                    return false;
                }
            }
        }
