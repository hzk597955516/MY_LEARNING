...
public class Router {
    ...
    public static List<NavigationDirection> routeDirections(AugmentedStreetMapGraph g, List<Long> route) {
        ...
        WeightedEdge<Long> preEdge;
        WeightedEdge<Long> currEdge;

        if (ways.size() == 1) {
            navigation  = setNavigation(preDirection, ways.get(0).getName(), ways.get(0).weight());
            Navigation.add(navigation);
            return Navigation;
        }

        for (int i = 1; i < ways.size(); i += 1){
            preEdge = ways.get(i - 1);
            currEdge = ways.get(i);

            Long preNode = preEdge.from();
            Long currNode = preEdge.to();
            Long nextNode = currEdge.to();

            String preName = GetName(preEdge);
            String currName = GetName(currEdge);

            distance += preEdge.weight();

            if (!currName.equals(preName)){
                navigation = setNavigation(preDirection, preName, distance);
                Navigation.add(navigation);

                currDirection = GetDirection(GetBearing(g, preNode, currNode), GetBearing(g, currNode, nextNode));
                distance = 0;
                preDirection = currDirection;
            }

            if (i == ways.size() - 1){
                distance += currEdge.weight();
                navigation = setNavigation(preDirection, currName, distance);
                Navigation.add(navigation);
            }
        }
        return Navigation;
    ...
    public static double GetBearing(AugmentedStreetMapGraph g, Long PreNode, Long NowNode){
        double preLon = g.lon(PreNode);
        double preLat = g.lat(PreNode);

        double nowLon = g.lon(NowNode);
        double nowLat = g.lat(NowNode);

        return NavigationDirection.bearing(preLon, nowLon, preLat, nowLat);
    }

    public static int GetDirection(double preBearing, double currBearing){
        return NavigationDirection.getDirection(preBearing, currBearing);
    }

    public static String GetName(WeightedEdge<Long> edge){
    ...
    public static List<WeightedEdge<Long>> GetWays(AugmentedStreetMapGraph g, List<Long> router){
        List<WeightedEdge<Long>> ways = new LinkedList<>();
        Long currNode;
        Long nextNode;

        for (int i = 0; i < router.size() - 1; i += 1){
            currNode = router.get(i);
            nextNode = router.get(i + 1);
            for (WeightedEdge<Long> edge: g.neighbors(currNode)){
    ...
    public static NavigationDirection setNavigation(int direction, String name, double distance){
