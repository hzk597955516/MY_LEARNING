...
public class RasterAPIHandler extends APIRouteHandler<Map<String, Double>, Map<String, Object>> {
    ...
    public Map<String, Object> processRequest(Map<String, Double> requestParams, Response response) {
        ...
        int rasterULLonNum = calcRastereParamNum(depth, requestULLon, ROOT_ULLON, ROOT_LRLON, true);
        rasterULLon = calcRasteredParam(depth, rasterULLonNum, ROOT_ULLON, ROOT_LRLON, true, true);
        results.put("raster_ul_lon", rasterULLon);

        int rasterLRLonNum = calcRastereParamNum(depth, requestLRLon, ROOT_ULLON, ROOT_LRLON, false);
        rasterLRLon = calcRasteredParam(depth, rasterLRLonNum, ROOT_ULLON, ROOT_LRLON, false, true);
        results.put("raster_lr_lon", rasterLRLon);

        int rasterULLatNum = calcRastereParamNum(depth, requestULLat, ROOT_ULLAT, ROOT_LRLAT, true);
        rasterULLat = calcRasteredParam(depth, rasterULLatNum, ROOT_ULLAT, ROOT_LRLAT, true, false);
        results.put("raster_ul_lat", rasterULLat);

        int rasterLRLatNum = calcRastereParamNum(depth, requestLRLat, ROOT_ULLAT, ROOT_LRLAT, false);
    ...
    private int calcRastereParamNum(int depth, double requestParam, double rootUL, double rootLR, boolean isUL){
        int bound = (int) (Math.pow(2, depth) - 1);

        double tilesize = Math.abs(rootLR - rootUL) / (bound + 1);

        double temp = Math.abs(requestParam - rootUL) / tilesize;

        int rastereParaNum = (int) Math.floor(temp);

        if (rastereParaNum > bound){
            rastereParaNum = bound;
        }
        else if (rastereParaNum < 0){
            rastereParaNum = 0;
        }

        return rastereParaNum;
    }

    private double calcRasteredParam(int depth, int ratereParamNum, double rootUL, double rootLR, boolean isUL, boolean isLon){
        int bound = (int) (Math.pow(2, depth) - 1);
        ...
        if (isUL){
            if (isLon){
                rasteredParam = rootUL + ratereParamNum * tilesize;
            }
            else {
                rasteredParam = rootUL - ratereParamNum * tilesize;
            }
            ...
            if (isLon) {
                rasteredParam = rootUL + (ratereParamNum + 1) * tilesize;
            }
            else {
                rasteredParam = rootUL - (ratereParamNum + 1) * tilesize;
            }
