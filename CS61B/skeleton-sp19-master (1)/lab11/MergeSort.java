...
public class MergeSort {
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(
            Queue<Item> items) {
        // Your code here!
        Queue<Queue<Item>> singels = makeSingleItemQueues(items);
        while (singels.size() != 1){
            singels.enqueue(mergeSortedQueues(singels.dequeue(), singels.dequeue()));
        }
        items = singels.dequeue();
        return items;
    }
}
