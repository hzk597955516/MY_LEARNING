...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operetor = scheme_eval(first, env)
        operands = rest.map((lambda x: scheme_eval(x, env)))
        validate_procedure(operetor)
        return scheme_apply(operetor, operands, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        p = scheme_eval(expressions.first, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        fun = LambdaProcedure(target.rest, expressions.rest, env)
        env.define(target.first, fun)
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if names is nil:
            names = Pair(binding.first, nil)
            values = Pair(scheme_eval(binding.rest.first, env), nil)
            n = names
            v = values
        else:
            n.rest = Pair(binding.first, nil)
