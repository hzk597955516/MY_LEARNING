...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    def find_row(board, max_rows, column):
        if max_rows == 0:
            return -1
        elif get_piece(board, max_rows-1,column) == '-':
            return max_rows-1
        else:
            return find_row(board, max_rows-1, column)
    row = find_row(board, max_rows, column)
    if row == -1:
        new_board = board
    else:
        h_board = replace_elem(board[row], column, player)
        new_board = replace_elem(board, row, h_board)
...
def print_board(board, max_rows, max_cols):
    ...
    for i in range(max_rows):
        d = ''
        for r in range(max_cols):
            d = d + get_piece(board, i, r) + ' '
        print(d.strip())
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    horizontally = check_win_row(board, max_rows, max_cols, num_connect, row, player)

    vertically = check_win_column(board, max_rows, max_cols, num_connect, col, player)
    if diagonal_win or horizontally or vertically:
        return True
    else:
        return False
...
def play(board, max_rows, max_cols, num_connect):
    ...
    while True:
        ...
        while True:
            try:
                col_index = int(input('请决定你的列数 {}? '.format(who)))
...
def start_game():
    ...
    while True:
        ...
        while True:
            try:
                num_connect = int(input('需要几颗相邻的棋子才能胜利? '))
            except ValueError as e:
                print('无效输入，请再试一遍.')
                continue
            break
