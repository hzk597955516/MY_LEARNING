...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            ...
            score0 += take_turn(num_rolls, opponent_score, dice)
            if is_swap(score0, score1):
                m = score0
                score0 = score1
                score1 = m
        if player == 1:
            num_rolls = strategy0(score1,score0)
            opponent_score = score0
            score1 += take_turn(num_rolls, opponent_score, dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0,score1):
        ...
        if who == 0:
            ...
            ps = score1
        if lock:
            sta = "{} points! That's the biggest gain yet for Player {}".format(eva,who)
            print(sta)
        return announce_highest(who,previous_high=ph,previous_score=ps)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    times = 0
    while roll > 0:
        methods = make_averaged(roll_dice,num_samples)
        score = methods(roll,dice)
        if score > high_score:
            high_score = score
            times = roll
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    af_score = score + free_bacon(opponent_score)
    if is_swap(af_score, opponent_score):
        if opponent_score > af_score:
            return 0
        else:
            return num_rolls
